...
MODULES            := power_counter
SCRIPTS            := powercounter
...
# ----------------------------------------------------------------------------
help:
	@echo "Makefile for powercounter"
	@echo "========================="
	@echo
	...
	@echo " pyinstaller               : Generate dist/powercounter distributable."
	@echo " pyinstaller-test          : Test the dist/powercounter distributable"
	...
	@echo " run                       : Run 'powercounter' with the correct"
	@echo "                             PYTHONPATH variable. All remaining"
	@echo "                             arguments are forwarded to powercounter."
...
run:
	@source venv/bin/activate; \
	PYTHONPATH=$(PYTHONPATH) ./powercounter $(RUN_ARGS)
...
unittests-coverage:
	...
	@PYTHONPATH=$(PYTHONPATH) nosetests -v -w tests/unittests --with-coverage \
        --cover-package=power_counter --cover-erase --cover-min-percentage=80 \
	--cover-inclusive \
        --cover-branches \
...
pyinstaller: dist/powercounter

dist/powercounter:
	@pyinstaller powercounter --onefile

pyinstaller-test: dist/powercounter
	@tests/functional_tests/run_tests.sh -p

build-release: releases/powercounter_Ubuntu16.04_amd64 releases/powercounter_Ubuntu18.04_amd64 releases/powercounter_Ubuntu20.04_amd64

releases/powercounter_Ubuntu16.04_amd64:
	@mkdir -p releases
	@docker run --rm \
...
releases/powercounter_Ubuntu18.04_amd64:
	@mkdir -p releases
	@docker run --rm \
	-e TGTUID=$(shell id -u) -e TGTGID=$(shell id -g) \
	-v $(PWD):/workdir \
...
releases/powercounter_Ubuntu20.04_amd64:
	@mkdir -p releases
	@docker run --rm \
	-e TGTUID=$(shell id -u) -e TGTGID=$(shell id -g) \
	-v $(PWD):/workdir \
